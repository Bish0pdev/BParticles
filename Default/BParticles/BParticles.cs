...
namespace BParticleTemplate.Default.Packages
{
    public class ParticleSystem
    {
        ...
        public Particle AddParticle(List<ParticleSpawnModifier> modifiers)
        {
        ...
        public List<Particle> ParticlesInRadius(Vector2 vector2, float radius)
        {
            List<Particle> list = new List<Particle>();
            foreach (Particle item in particles)
            {
                if(Vector2.Distance(item.Position, vector2) < radius)
                {
                    list.Add(item);
                }
            }
